...
build-docker: ## Build the docker image
	docker build -t rawdata-converter-app-xml:dev -f Dockerfile .
...
changelog: ## Generate CHANGELOG.md
	github_changelog_generator -u statisticsnorway -p rawdata-converter-app-xml
...
.PHONY: run-local-gcp
run-local-gcp: ## Run the app locally (without docker) using GCP services
